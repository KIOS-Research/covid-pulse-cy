...
;(function($) {
    $.fn.formset = function(opts)
    {
        var options = $.extend({}, $.fn.formset.defaults, opts),
            ...
            insertDeleteLink = function(row) {
                if (row.is('TR')) {
                    ...
                    row.children(':last').append('<button type="button" class="btn btn-danger ' + options.deleteCssClass +'">' + options.deleteText + '</button>');
                } else if (row.is('UL') || row.is('OL')) {
                    // If they're laid out as an ordered/unordered list,
                    // insert an <li> after the last list item:
                    row.append('<li><button type="button" class="btn btn-danger ' + options.deleteCssClass +'">' + options.deleteText + '</button></li>');
                } else {
                    // Otherwise, just insert the remove button as the
                    // last child element of the form's container:
                    row.append('<button type="button" class="btn btn-danger ' + options.deleteCssClass +'">' + options.deleteText + '</button>');
                }
        ...
        if ($$.length) {
            ...
            if ($$.attr('tagName') == 'TR') {
                ...
                $$.parent().append('<tr><td colspan="' + numCols + '"><button type="button" class="btn btn-primary ' + options.addCssClass + '">' + options.addText + '</button></tr>');
                addButton = $$.parent().find('tr:last a');
                addButton.parents('tr').addClass(options.formCssClass + '-add');
            } else {
                // Otherwise, insert it immediately after the last form:
                $$.filter(':last').after('<button type="button" class="btn btn-primary ' + options.addCssClass + '">' + options.addText + '</button>');
    ...
    $.fn.formset.defaults = {
        ...
        addText: 'Προσθήκη',          // Text for the add link
        deleteText: 'Διαγραφή',            // Text for the delete link
